sap.ui.define([
	...
	return BaseController.extend("myspace.ProductList.controller.Worklist", {

		formatter: formatter,
		
		_mFilters: {
			cheap: [new Filter("UnitPrice","LT",100)],
			moderate: [ new Filter("UnitPrice","BT",100, 1000)],
			expensive: [new Filter ("UnitPrice","GT",1000)]
		},
		...
		onUpdateFinished : function (oEvent) {
			...
			if (iTotalItems && oTable.getBinding("items").isLengthFinal()) {
				...
				// get the count for each filters
		...
		_getPopover: function(){
			if(!this._oPopover){
				//create a popover via fragment factory
				this._oPopover = sap.ui.xmlfragment("myspace.ProductList.view.Popover",this);
		...
		_showObject : function (oItem) {
			this.getRouter().navTo("object", {
				objectId: oItem.getBindingContext().getProperty("ProductKey")
			});
		},
